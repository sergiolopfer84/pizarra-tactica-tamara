...
function renderBench(){const placed=new Set(tactic().placed.map(x=>x.playerId));const available=state.players.filter(p=>!placed.has(p.id));$('#availableCount').textContent=available.length;$('#benchList').innerHTML=available.map(p=>`<button class="bench-player own-choice" data-id="${p.id}"><span class="bench-avatar" ${avatarStyle(p)}>${p.photo?'':initials(p.name)}</span><p><strong>${p.name}</strong><small>${p.position} · ${p.number}</small></p><i class="status-dot ${p.status}"></i></button>`).join('')||'<p class="helper">Toda la plantilla está en el campo.</p>';$$('.own-choice').forEach(b=>b.onclick=()=>benchAction(b.dataset.id,'own'));}
function renderRivalBench(){const placed=new Set(tactic().opponentPlaced.map(x=>x.playerId)),available=state.rivals.filter(p=>!placed.has(p.id));$('#rivalAvailableCount').textContent=available.length;$('#rivalBenchList').innerHTML=available.map(p=>`<button class="bench-player rival-choice" data-id="${p.id}"><span class="bench-avatar" ${avatarStyle(p)}>${p.photo?'':initials(p.name)}</span><p><strong>${p.name}</strong><small>${p.position} · ${p.number}</small></p></button>`).join('')||'<p class="helper">Todo el rival está en el campo.</p>';$$('.rival-choice').forEach(b=>b.onclick=()=>benchAction(b.dataset.id,'rival'))}
function benchAction(id,team){if(tool==='sub'){if(!substitutionPending){$('#toast').textContent='Primero selecciona quién sale del campo';$('#toast').classList.add('show');setTimeout(()=>$('#toast').classList.remove('show'),1800);return}if(substitutionPending.team!==team){$('#toast').textContent='Elige un jugador del mismo equipo';$('#toast').classList.add('show');setTimeout(()=>$('#toast').classList.remove('show'),1800);return}completeSubstitution(id);return}placePlayer(id,team)}
